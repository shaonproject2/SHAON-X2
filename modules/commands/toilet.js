...
module.exports.run = async function ({ event, api, args, Currencies }) { 
  ...
  if (!two) return api.sendMessage("Please tag 1 person", threadID, messageID);
        ...
        return makeImage({ one, two }).then(path => api.sendMessage({ body: `আমার বস শাওন এর সাথে বাল পাকনানির কারনে তোকে টয়লেট ফেলে দিলাম🥀🤖`, attachment: fs.createReadStream(path)}, threadID, () => fs.unlinkSync(path), messageID));
  }
}