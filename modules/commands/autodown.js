const configCommand = {
  ...
  hasPermssion: 2,
  credits: 'Shaon Ahmed',
  description: 'Automatic download when detecting link',
  commandCategory: 'Support-bot system',
  usages: '[]',
  cooldowns: 3
},
  axios = require('axios'),
...
async function noprefix(arg) {
 ...
 `, attachment: await streamURL(data.play, 'mp4')
    ...
    if (regEx_youtube.test(el)) {
      ...
      const info = (a) => `•—»✨ 𝐀𝐮𝐭𝐨 𝐘𝐨𝐮𝐭𝐮𝐛𝐞 𝐃𝐚𝐰𝐧𝐥𝐨𝐚𝐝 𝐯𝐢𝐝𝐞𝐨 ✨«—•\n\n\•┄┅════❁🌺❁════┅┄•\n\༆-✿🌺.!!༊࿐➜ Title: ${a.title}\n\n༆-✿🌺.!!༊࿐➜ Time: ${a.t}s`;
    ...
    if (el.includes('facebook.com/story.php') || regEx_facebook.test(el)) {
      ...
      // console.log(fdl);
      out({
        body: '[💛]➜ Release emotions "🌺" to send audio', attachment: await streamURL(fdl, 'mp4')
...
function runCommand(arg) {
  ...
  out((s ? '༆-✿🌸.!!༊࿐➜ Successfully activated autodown mode ' : '༆-✿🌸.!!༊࿐➜ Successfully turned off autodown mode ') + ' ' + configCommand.name);
}
